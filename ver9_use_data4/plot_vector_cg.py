...
# print(traj[-1].particles.velocity)
# print(len(traj))
...
plt.figure(figsize=(10,10*ly/lx),dpi=300)
...
for t in range(len(traj)-1,0,-3):
    ...
    rx_list,ry_list,vx_list,vy_list=cg(pos,vel,cg_l_g,cg_ngx,cg_ngy,ave_flow)

    for i in range(len(rx_list)):
        norm=vx_list[i]*vx_list[i]+vy_list[i]*vy_list[i]
        vx_list[i]/=norm
        vy_list[i]/=norm
